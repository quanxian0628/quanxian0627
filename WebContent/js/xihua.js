$(document).ready(function () {
	...
	$(".btnRMSave").click(function () {
		//alert(global_arr.sort().toString()+"==="+global_arr2.sort().toString());
		var arr = global_arr.sort().toString();
		var arr2 = global_arr2.sort().toString();
		matchId2=0;

		if (global_rId == 0 && arr2 == "") {
			alert("请为角色分配权限！");
			return;
		}
		//alert(global_arr.sort().toString()+"==="+global_arr2.sort().toString());
		...
		if (arr == arr2) {
			...
			$.post("ExistsRIdForRMServlet", "rId=" + global_rId, function (data) {
				if (data == "true") {
					//先删除，再添加
					delRidByRM(global_rId);
					addRidByRM(global_rId,global_arr2);
				} else {
					//直接添加
					addRidByRM(global_rId,global_arr2);
				}
	...
	function delRidByRM(rId) {
		$.post("DelRidByRMServlet", "rId="+rId, function (data) {
			if(data=="true"){
				//alert("更新成功！");
				//arr2=new Array();
				cha2();
	...
	function addRidByRM(rId,arr2) {
		if(arr2==""){
			return;
		}
		$.post("AddRidForRMServlet", "rId="+rId+"&arr2="+arr2, function (data) {
			if(data=="true"){
				alert("更新成功！");
			}else{
				alert("更新失败！");
